%This code takes a run of Julie Onton experiment and segregates 
%letter-encoded data intervals from non-encoded data intervals, then stores 
%them in a cell.
% After performing eDMD+rFf on the data, an array of "dominant mode
% frequencies is associated to a response variable (whether a letter is 
% encoded or not), so a logistic regression algorithm can be implemented 
% later:

numSegments = numel(pSegments); %pSegments cardinality
results = cell(numSegments, 1);  % store eDMD outputs here

%for k = 1:numSegments
for k = 1:1
    dataM = pSegmentsplus{k};        % matrix for this segment
%results{k} = eDMD(M);    % run eDMD:
%Parameters
sigma = 1;
nstacks = 5;                                          %Stacking parameter
r =1300; %355;                                              %SVD: rank truncation 
%N = 2;                 %ARTIFICIAL DATASET: number of channels in the data
N = size(dataM,1);                 %ARTIFICIAL DATASET: number of channels in the data
%M = 160;              %ARTIFICIAL DATASET: number of snapshots in the data
M = size(dataM,1);              %ARTIFICIAL DATASET: number of snapshots in the data
...
[fsorted,I] = sort(f);
Psorted = P(I);
%Array of peak power frequencies:
[pks,locs] = findpeaks(Psorted,fsorted);
figure;
plot(fsorted)
...
title('artificial dataset', sigma)
subplot(2,1,2)
