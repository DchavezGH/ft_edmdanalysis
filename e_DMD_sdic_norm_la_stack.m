...
clear X_0
%ARTIFICIAL DATASET:
X_osc = 10.* sin(repmat(0.1.*t, N, 1)); %This is a sine oscillation, comment it when not needed
Xconst = zeros(N,M);
Xconst (1:10, :) = 5;
...
%Here we will try stacking, that's why we don't include X1 and X2 below:
nstacks = 2;
if nstacks > 1
    Xaug = [];
    for st = 1:nstacks
...
% A eDMD dictionary made of the 3 first Hermite Polynomials.
...
%We scaled back the higher hermite pols
H0 = ones(size(X1));                           
H1 = 2 * X1;   
H2 = 0.05.*(4 * X1.^2 - 2);                            
H3 = 0.005.*(8 * X1.^3 - 12 * X1);                      
X1e=[H0; H1; H2; H3];
clear H0 H1 H2 H3
H0 = ones(size(X2));                           
H1 = 2 * X2; 
H2 = 0.5.*(4 * X2.^2 - 2);                            
H3 = 0.005.*(8 * X2.^3 - 12 * X2);                       
X2e=[H0; H1; H2; H3];
clear H0 H1 H2 H3
Psi_mX = X1e.';
Psi_mY = X2e.';
...
% %%with no success yet...
  ...
  r=180; % rank truncation parameter
  u_t=u(:,1:r); 
  sig_t=sig(1:r,1:r); 
  v_t=v(:,1:r); % r-truncate U, Sigma & V
  Kt =u_t*sig_t*v_t' ;   %eDMDSVD: generates the truncated Kt


%Calculate the and normalize eigenvectors: Working now :)
[XI,MU,W] = eig(Kt);        %Generate eigenvalues and left-right eigenvctors
W=inv(XI)'; %not helping...
%MU(1,1) = 0; %Very experimental notion of disregarding the first eigenvalue
%MU(2,2) = 0;
%MU(3,3) = 0;
%MU(4,4) = 0;
%MU(5,5) = 0;
%MU(6,6) = 0;
%MU(7,7) = 0;
XI(XI< 0.00000000001) = 0;                     %Remove all neglible entries 
