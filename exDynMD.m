...
%SET DMD PARAMETERS:
r=200; %% rank truncation smaller r finish fast at the expense of precision
dt = 0.004; % delta*t Considering a sampling rate of 250Hz
ch = size(X_0, 1);  %number of channels in the data
M = size(X_0, 2); %number of data points in the data
t = dt.*(1:M); %Time vector

%ARTIFICIAL DATASET:
X_osc = 100.* sin(repmat(0.1.*t, 71, 1)); %This is a sine oscillation, comment it when not needed
Xconst = zeros(ch,M);
Xconst (1:10, :) = 10;
...
%X1e=[X1; H0; H1; H2; H3; H4; H5; H6; H7; H8; H9; H10]; Since X1 is a
%multiple of H1, the dictionary would not be inearly independent, corrected
%dictionary below:
...
K = size(X2e, 1);  %number of channels in the data
btop = zeros(ch,ch);
%bmid= 0.5.*eye(ch);
bmid= eye(ch)./2;
blow = zeros(K-2*ch,ch);
B=[btop;bmid;blow];
...
%Remember:X1e and X2e are Psi_M transposed
G=(X1e*X1e.')/M;
A=(X1e*X2e.')/M;
K=pinv(G)*A; %The eDMD approximation to the Koopman operator
            %This is a nice place to introduce SVD'..
[XI,MU,W] = eig(K);

%Normalize the left eigenvectors:
inerp=sum(W.*XI);  %Calculate inner product across columns
Wn = W./inerp;      % then divide w_i by such inner product to normalize them
inerpchk=sum(Wn.*XI); %inerpchk should be a vector of 1s if all went ok. 
                        %Note that very small eigenvalues wil introduce inestability that result in 
                        %erroneous normalizations. WE NEED TO ADDRESS IT!
